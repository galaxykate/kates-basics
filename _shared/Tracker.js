  ...
  */
	class Hand extends Trackable {
	  ...
	  static {
		...
		constructor(tracker, dimensionality) {
			...
			this.fingerTips = this.fingers.map(f => f.tip)
	...
	class Tracker {
		constructor({
			...
			drawSource({p, flip=false, x = 0, y = 0, scale = 1.0, opacity=1.0, blendMode}) {
			...
			get activeHands() {
				return this.hands.filter(tr => tr.isActive)
			}

			get activeHandPoints() {
				return this.hands.filter(tr => tr.isActive).map(h => h.landmarks).flat()
			}

			get activeFingerTips() {
				return this.hands.filter(tr => tr.isActive).map(h => h.fingers.map(finger => finger.joints[3]).flat()).flat()
			}
